      ...
      {u.combatStats && Object.values(u.combatStats).some((v) => v != null) && (
        <div style={{ marginBottom: 6 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1px 12px", fontSize: 11.5 }}>
            {[
              ["HP", u.combatStats.hp], ["ATK", u.combatStats.atk], ["DEF", u.combatStats.def],
              // Accanto alla SPD si mostra quanta ne arriva dalle rune: è il
              // pezzo su cui si lavora quando si fa speed tuning, il resto è
              // base del mostro e non si tocca (14/08/2026, Flora).
              ["SPD", u.combatStats.spd ?? u.combatStats.spdCombat, u.combatStats.spdFromRunes],
            ...
            ].map(([label, val, daRune]) => val != null && (
              <div key={label} style={{ display: "flex", justifyContent: "space-between", color: "var(--text-muted)" }}>
                <span>{label}</span>
                <span style={{ color: "var(--text)" }}>
                  {val}
                  {daRune ? <span style={{ color: "var(--green)", opacity: 0.85 }}> (+{daRune})</span> : null}
                </span>
              </div>
            ))}
          </div>
