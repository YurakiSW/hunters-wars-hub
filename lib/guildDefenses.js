...
export async function importGuildDefenseLog(rawLogText, { importedByNickname, label }) {
  ...
  // Dettaglio per gilda nemica: se un log copre più round di siege (date/
  // avversari diversi mischiati in un solo incolla), lo storico deve poter
  // mostrare OGNI matchup separato — data e gilda affrontata — non un'unica
  // riga con tutte le gilde nemiche buttate insieme.
  const perGuild = new Map(); // guild -> { count, dateFrom, dateTo }
  const timestamps = [];
  ...
  for (const b of toStore) {
    ...
    const pg = perGuild.get(b.enemyGuild) || { guild: b.enemyGuild, count: 0, dateFrom: null, dateTo: null };
    pg.count++;
    if (b.timestamp) {
      pg.dateFrom = pg.dateFrom ? Math.min(pg.dateFrom, b.timestamp) : b.timestamp;
      pg.dateTo = pg.dateTo ? Math.max(pg.dateTo, b.timestamp) : b.timestamp;
    }
    perGuild.set(b.enemyGuild, pg);
  ...
  const logRecord = {
    ...
    ourGuildName: guildName,
    enemyGuilds: [...enemyGuilds],
    // Ordinato per data, non per conteggio: così un log con più round si
    // legge in ordine cronologico, matchup dopo matchup.
    enemyGuildBreakdown: [...perGuild.values()].sort((a, b) => (a.dateFrom || 0) - (b.dateFrom || 0)),
    dateFrom,
    dateTo,
  };
