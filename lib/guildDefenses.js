...
async function loadAllBattlesByDefense(included) {
  ...
  const pipeline = redis.pipeline();
  for (const dK of defenseKeys) pipeline.smembers(battlesByDefenseKey(dK));
  const idSets = await pipeline.exec(); // stesso ordine di defenseKeys

  const idsByDefense = new Map();
  const allIds = [];
  for (let i = 0; i < defenseKeys.length; i++) {
    const ids = idSets[i] || [];
    idsByDefense.set(defenseKeys[i], ids);
    allIds.push(...ids);
  }
  if (!allIds.length) return new Map();

  const allRecords = await redis.mget(...allIds.map(battleRecordKey));
  const byId = new Map();
  allIds.forEach((id, i) => { if (allRecords[i]) byId.set(id, allRecords[i]); });

  const out = new Map();
  for (const [dK, ids] of idsByDefense) {
    const battles = ids.map((id) => byId.get(id)).filter(Boolean).filter((b) => included.has(b.siegeKey));
...
export async function listGuildDefenses(searchQuery) {
  ...
  const byDefense = await loadAllBattlesByDefense(included);
  const out = [];
...
export async function getGuildDefenseDetail(defenseKey) {
  ...
  const records = await redis.mget(...ids.map(battleRecordKey));
...
export async function getTeamDetail(teamKey) {
  ...
  const players = [];
  for (const [dK, battles] of byDefense) {
    if (teamKeyFromDefenseKey(dK) !== teamKey) continue;
...
export async function archiveCurrentSeason(siegeKeysToArchive) {
  ...
  const byDefense = await loadAllBattlesByDefense(included);
  const defenses = [...byDefense.entries()].map(([dK, battles]) => summarizeDefenseBattles(dK, battles));
