...
export function extractRichReplayDetails(rawLogText) {
  ...
  for (const json of extractResponsesFor(rawLogText, "GetGuildSiegeBattleReplayData")) {
    ...
    const offenseRunes = attacker.battle_units.map((bu) => (bu.unit.runes || []).map((r) => r.set_id));
...
// varianti dello stesso counter), due squadre con leader diverso sono
// considerate offence DIVERSE.
