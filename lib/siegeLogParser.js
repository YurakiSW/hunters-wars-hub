...
export function extractRichReplayDetails(rawLogText) {
  ...
  for (const json of extractResponsesFor(rawLogText, "GetGuildSiegeBattleReplayData")) {
    ...
    // Stat BASE pure dal replay (con/atk/def/spd/resist) — CORRETTO
    // 30/07/2026: NON sono già calcolate con rune/set/artefatti/leader/torri
    // come si pensava prima (verificato con screenshot reali: il replay dà
    // sempre e solo il numero "bianco", mai il totale con rune). Il totale
    // vero si ricalcola a mano in lib/runeSets.js computeCombatStats(),
    // sommando anche artefatti e relic. Usate comunque anche per l'ordine
    // turni (SPD base, sufficiente per quello).
    ...
    if (!byKey.has(key)) byKey.set(key, { offenseIds, offenseRunes, offenseArtifacts, offenseRelics, offenseSpd, offenseCombatBase });
  }
  return byKey;
}
