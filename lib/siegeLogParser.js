...
export function extractRichReplayDetails(rawLogText) {
  // Ogni chiave "offenseIds::defenseIds" ora tiene un ARRAY di TUTTE le
  // catture trovate (una per giocatore/replay), non più una sola scelta
  // arbitraria — corretto l'11/08/2026 (Flora): con più giocatori che
  // usano la stessa squadra, tenerne solo una buttava via le rune di tutti
  // gli altri, e quale sopravviveva dipendeva dall'ordine nel file, non da
  // chi vinceva di più. Chi usa questo dato decide poi lui come sceglierne
  // una (vedi recordCrossPlayerBattles in lib/siegeStats.js).
  ...
  for (const json of extractResponsesFor(rawLogText, "GetGuildSiegeBattleReplayData")) {
    ...
    // Stat BASE pure della specie (con/atk/def/spd/resist). Il replay NON
    // contiene mai i totali finali con rune/artefatti/relic: quelli si
    // ricostruiscono in lib/runeSets.js computeCombatStats(). La SPD qui
    // è quella base, sufficiente per l'ordine turni.
    ...
    // Relic (7° slot, dal 2026): al massimo 1 per mostro, dentro unit.relics.
    // Salviamo l'array grezzo intero (non solo pri_effect) per coerenza con
    // rune/artefatti — la traduzione (solo main stat, per ora) avviene dopo
    // in lib/relicEffects.js, così se in futuro serve anche l'effetto unico
    // non serve ricaricare il log.
    const offenseRelics = attacker.battle_units.map((bu) => bu.unit.relics || []);
    // Nick del giocatore che ha usato questa squadra in attacco: serve per
    // attribuire il counter a chi l'ha davvero giocato invece di mostrare
    // solo un generico "Siege Log".
    const offenseWizardName = attacker.wizard_name || null;
    // Leader della squadra, dal replay. Nei 462 team del log 11/08 coincide
    // sempre col primo mostro, ma leggerlo davvero è più sicuro che fidarsi
    // di una regolarità — e serve per applicare la leader skill, che in
    // Siege alza le stat di tutta la squadra (14/08/2026, Flora).
    const offenseLeaderId = attacker.leader_unit?.unit_master_id ?? null;

    // Impronta univoca di QUESTA battaglia: i rand_seeds sono i numeri
    // casuali generati per quel singolo scontro (verificato l'12/08/2026 su
    // 50 replay reali: sempre presenti), quindi due battaglie diverse non
    // possono averli uguali, e la STESSA battaglia li ha identici anche se
    // il log viene reimportato. Serve a non contare due volte la stessa
    // vittoria per una build quando si ricarica un log già visto.
    const seeds = parsed?.battle_info?.rand_seeds;
    const captureId = Array.isArray(seeds) && seeds.length && attacker.wizard_id != null
      ? `cap:${attacker.wizard_id}:${seeds.join(",")}`
      : null;

    const key = `${idsKey(offenseIds)}::${idsKey(defenseIds)}`;
    const capture = { offenseIds, offenseRunes, offenseArtifacts, offenseRelics, offenseSpd, offenseCombatBase, offenseWizardName, offenseLeaderId, captureId };
    if (!byKey.has(key)) byKey.set(key, []);
    // Stesso identico giocatore con la stessa identica build può comparire
    // più volte nello stesso log (più vittorie con la stessa squadra) — non
    // lo aggiungiamo due volte, altrimenti gli conterebbe doppio più avanti.
    const list = byKey.get(key);
    const dupHash = JSON.stringify({ n: offenseWizardName, r: offenseRunes });
    if (!list.some((c) => JSON.stringify({ n: c.offenseWizardName, r: c.offenseRunes }) === dupHash)) {
      list.push(capture);
    }
  }
  return byKey;
...
export function extractBattleEntries(rawLogText) {
  ...
  for (const json of extractResponsesFor(rawLogText, "GetGuildSiegeBattleLogByWizardId")) {
    for (const log of json?.log_list || []) {
      for (const b of log?.battle_log_list || []) {
        ...
        // Nome di chi ha attaccato: è il proprietario del Battle Log. Serve
        // per poter dire CHI ha perso su una certa difesa — le sconfitte non
        // hanno un replay da cui ricavare la build, ma il nome sì
        // (14/08/2026, Flora).
        entries.push({ offenseIds: deck[0], defenseIds: deck[1], win: b.win_lose === 1, battleId, wizardName: log?.wizard_name || null });
      }
    }
  }
  ...
  for (const json of extractResponsesFor(rawLogText, "getBattleReplayInfo")) {
    ...
    for (let w = 0; w < info.result_list.length; w++) {
      ...
      entries.push({ offenseIds: info.unit_list[w], defenseIds: info.opp_unit_list[w], win: info.result_list[w] === 1, battleId, wizardName: info.wizard_name || null });
    }
  }
...
export function entriesToMatchups(entries, monsterByComId) {
  ...
  for (const e of entries) {
    ...
    matchups.push({ offense: offenseNames, defense: defenseNames, win: e.win, battleId: e.battleId || null, wizardName: e.wizardName || null });
  }
  return matchups;
}
