...
export function extractRichReplayDetails(rawLogText) {
  ...
  for (const json of extractResponsesFor(rawLogText, "GetGuildSiegeBattleReplayData")) {
    ...
    if (!byKey.has(key)) byKey.set(key, { offenseIds, offenseRunes, offenseArtifacts, offenseRelics, offenseSpd, offenseCombatBase, offenseWizardName });
  }
  return byKey;
}
...
// Estrae le battaglie in cui la NOSTRA GILDA è sotto attacco (a differenza
// di extractRichReplayDetails, che guarda solo chi ha VINTO per costruire i
// Counter). Qui il criterio è diverso: guardiamo il `guild_name` esatto, non
// chi ha vinto — così troviamo sia le difese TENUTE (vinte) sia quelle ROTTE
// (perse), che è proprio il punto di questa funzione.
//
// Un replay senza nessun lato della gilda cercata (entrambe le squadre
// esterne, es. due nemici che si scontrano) viene scartato.
export function extractGuildDefenseBattles(rawLogText, guildName) {
  const battles = [];
  for (const json of extractResponsesFor(rawLogText, "GetGuildSiegeBattleReplayData")) {
    const b64 = json?.replay_data;
    if (!b64) continue;
    ...
    const users = parsed?.battle_info?.battle_users;
    if (!users || users.length < 2) continue;
    const ours = users.find((u) => u.guild_name === guildName);
    const enemy = users.find((u) => u.guild_name !== guildName);
    if (!ours || !enemy) continue;

    const toUnits = (side) =>
      side.battle_units.map((bu) => ({
        unitMasterId: bu.unit.unit_master_id,
        isLeader: side.leader_unit?.unit_master_id === bu.unit.unit_master_id,
        rawRunes: bu.unit.runes || [],
        rawArtifacts: bu.unit.artifacts || [],
        rawRelics: bu.unit.relics || [],
        rawCombatBase: {
          hp: bu.unit.con ?? null, atk: bu.unit.atk ?? null, def: bu.unit.def ?? null,
          spd: bu.unit.spd ?? null, resistance: bu.unit.resist ?? null,
        },
    ...
    // Nessun ID di battaglia univoco a livello del replay: costruiamo una
    // chiave di deduplica dai dati stessi. `tvaluelocal` (timestamp reale
    // della risposta, in secondi) la rende praticamente unica — verificato
    // su log reali che è un Unix timestamp valido, non un contatore interno.
    const dedupeKey = [
      ours.wizard_id, enemy.wizard_id, ours.win_lose,
      [...toUnits(ours).map((u) => u.unitMasterId)].sort((a, b) => a - b).join(","),
      json.tvaluelocal,
    ].join("|");

    battles.push({
      dedupeKey,
      timestamp: json.tvaluelocal || null,
      ownerWizardId: ours.wizard_id,
      ownerNick: ours.wizard_name,
      held: ours.win_lose === 1,
      ourUnits: toUnits(ours),
      enemyGuild: enemy.guild_name,
      enemyWizardId: enemy.wizard_id,
      enemyWizardName: enemy.wizard_name,
      enemyUnits: toUnits(enemy),
    });
  }
  return battles;
}
