...
export function extractRichReplayDetails(rawLogText) {
  ...
  for (const json of extractResponsesFor(rawLogText, "GetGuildSiegeBattleReplayData")) {
    ...
    // SPD combat (base + rune, già calcolata dal gioco) — serve per capire
    // in che ordine agiscono davvero in battaglia, non l'ordine con cui
    // compaiono nel log.
    const offenseSpd = attacker.battle_units.map((bu) => bu.unit.spd ?? 0);
    // Ogni runa: set_id (per il nome del set) + slot_no/pri_eff (per la
    // stat principale, es. "SPD" allo slot 2) — solo i 3 campi che servono,
    // non l'intero oggetto runa.
    const offenseRunes = attacker.battle_units.map((bu) =>
      (bu.unit.runes || []).map((r) => ({ set_id: r.set_id, slot_no: r.slot_no, pri_eff: r.pri_eff }))
    );
    ...
    if (!byKey.has(key)) byKey.set(key, { offenseIds, offenseRunes, offenseArtifacts, offenseSpd });
  }
  return byKey;
}
