...
// Elenco dei mostri comparsi per la PRIMA volta nell'ultimo sync (ID mai
// visti in nessun sync precedente) — aggiunto il 05/08/2026 (Flora) come
// sostituto onesto del riconoscimento automatico dei collab: non sappiamo
// dire "è un collab", ma sappiamo dire "è nuovo, dagli un'occhiata",
// senza scoprirlo per caso mesi dopo come è successo con Übel.
export async function getNewMonstersSinceLastSync() {
  return (await redis.get("monsters:newSinceLastSync")) || [];
}

export async function setNewMonstersSinceLastSync(list) {
  await redis.set("monsters:newSinceLastSync", list);
}
...
export async function setTwinsBulk(entries) {
  const twins = await getTwins();
  let saved = 0;
  for (const { altName, canonicalName } of entries || []) {
    const alt = (altName || "").trim();
    const canonical = (canonicalName || "").trim();
    if (!alt || !canonical) continue;
    if (normalizeMonsterName(alt) === normalizeMonsterName(canonical)) continue;
    twins[normalizeMonsterName(alt)] = canonical;
    saved++;
  }
  await redis.set(TWINS_KEY, twins);
  return { saved, total: Object.keys(twins).length };
}
...
// Lista di esclusione "non tenere mai il nome pulito" per com2us_id
// specifici, verificati a mano uno per uno (mai un'automatismo silenzioso
// su tutto il bestiario — lezione imparata il 05/08/2026). Salvata su
// Redis invece che scritta nel codice: aggiungerne uno nuovo da Admin ha
// effetto dal sync successivo, senza bisogno di un nuovo pacchetto.
export async function getNeverCleanIds() {
  return (await redis.get("monsters:neverCleanIds")) || [];
}

export async function addNeverCleanId(com2usId) {
  const list = await getNeverCleanIds();
  if (!list.includes(com2usId)) {
    list.push(com2usId);
    await redis.set("monsters:neverCleanIds", list);
  }
  return list;
}

export async function removeNeverCleanId(com2usId) {
  const list = (await getNeverCleanIds()).filter((id) => id !== com2usId);
  await redis.set("monsters:neverCleanIds", list);
  return list;
}
...
// Come getBaseAccuracyByName ma restituisce TUTTE le stat base che il replay
// non contiene, più la leader skill e l'elemento. Aggiunta il 14/08/2026
// (Flora): accuracy, crit rate e crit damage variano per mostro (Camilla
// parte da 30% di crit rate, Tesarion da 25% di accuracy) e la leader skill
// alza le stat di tutta la squadra in Siege.
export async function getMonsterBaseStatsByName() {
  const [synced, aliases] = await Promise.all([getSyncedMonsters(), getAliases()]);
  const map = new Map();
  for (const m of synced) {
    map.set(normalizeMonsterName(m.name), {
      accuracy: m.baseAccuracy ?? undefined,
      critRate: m.baseCritRate ?? undefined,
      critDamage: m.baseCritDamage ?? undefined,
      element: m.element ?? null,
      leaderSkill: m.leaderSkill ?? null,
    });
  }
  // I nomignoli di gilda puntano allo stesso mostro: stesse stat base.
  for (const [nickname, officialName] of Object.entries(aliases)) {
    const val = map.get(normalizeMonsterName(officialName));
    if (val) map.set(normalizeMonsterName(nickname), val);
  }
  return map;
}
