...
export function validateUnit(u) {
  ...
  } else if (!u.stats || u.stats.split("/").map((p) => p.trim()).filter(Boolean).length !== 3) {
    errors.push("Priorità statistiche incomplete (servono slot 2, 4 e 6, oppure spunta \"set libero\")");
  }
