...
import { listDefs, getCounter } from "./defs";
...
export async function findAutoAgainstDefs(units) {
  ...
  for (const def of defs) {
    const match = (def.counters || []).find(
      (c) => c.status === "approved" && compositionKey(c.units, canonicalMap) === key
    );
    if (match && !seen.has(def.id)) {
      seen.add(def.id);
      // sourceCounterId tiene il legame con il Counter che ha giustificato
      // questa voce — serve SOLO per poterla ritirare in modo sicuro se
      // quel counter sparisce (vedi refreshAgainstFromCounters). Le voci
      // aggiunte a mano (addAgainstEntry) non hanno questo campo e non
      // vengono MAI toccate in automatico, né qui né altrove.
      found.push({ id: newId("vs"), monsters: def.monsters, sourceCounterId: match.id });
    }
  }
  return found;
}
...
export async function createDeckFromCounter(counter, { authorId, authorNickname }) {
  ...
  // Chi ha davvero giocato quella build nei log di Siege. È più utile del
  // nome di chi ha premuto "crea deck": serve a sapere a chi chiedere le
  // rune. Resta finché qualcuno non modifica il deck a mano — da quel
  // momento il deck è farina del sacco di chi l'ha modificato e si mostra
  // il suo nome (14/08/2026, Flora).
  deck.logOwnerNickname = counter.logOwnerNickname || null;
  await redis.set(deckKey(deck.id), deck);
  await redis.sadd(DECK_IDS_KEY, deck.id);
  return deck;
}
...
// Bottone "🔄 Aggiorna da Counter" (Admin/Deck Builder, dalla pagina del
// deck stesso): aggiunge SOLO le Difese nemiche nuove per cui è comparso
// un Counter approvato con questa stessa identica squadra da quando il
// deck è stato creato/aggiornato l'ultima volta — non tocca mai le voci
// già presenti (manuali o auto-aggiunte alla creazione), non ne toglie
// mai nessuna. Zero automatismo silenzioso: parte solo quando qualcuno lo
// preme apposta.
// Bottone "🔄 Aggiorna da Counter" (Admin/Deck Builder, dalla pagina del
// deck stesso): aggiunge le Difese nemiche nuove per cui è comparso un
// Counter approvato con questa stessa identica squadra, E ritira quelle
// aggiunte in automatico il cui Counter di origine non esiste più o non è
// più approvato — ma SOLO quelle con un `sourceCounterId` tracciato (le
// voci aggiunte a mano, o quelle salvate prima che questo campo esistesse,
// non hanno questo legame e non vengono MAI toccate: nessuna rimozione
// "a caso" su qualcosa di cui non siamo sicuri all'origine).
export async function refreshAgainstFromCounters(deckId) {
  const deck = await redis.get(deckKey(deckId));
  if (!deck) return null;
  const canonicalMap = await getCanonicalNameMap();
  const key = compositionKey(deck.units, canonicalMap);

  const found = await findAutoAgainstDefs(deck.units);
  const current = deck.against || [];
  const existingKeys = new Set(current.map((a) => a.monsters.join("|").toLowerCase()));
  const newOnes = found.filter((f) => !existingKeys.has(f.monsters.join("|").toLowerCase()));

  const kept = [];
  let removed = 0;
  for (const a of current) {
    if (!a.sourceCounterId) { kept.push(a); continue; } // manuale o legacy: intoccabile
    const source = await getCounter(a.sourceCounterId);
    const stillValid = source && source.status === "approved" && compositionKey(source.units, canonicalMap) === key;
    if (stillValid) kept.push(a);
    else removed++;
  }

  if (!newOnes.length && !removed) return { added: 0, removed: 0 };
  const updated = { ...deck, against: [...kept, ...newOnes] };
  await redis.set(deckKey(deckId), updated);
  return { added: newOnes.length, removed };
}

// Stessa identica logica di refreshAgainstFromCounters, lanciata su OGNI
// deck del sito in un colpo solo — bottone "Aggiorna tutti" in ATK Deck.
export async function refreshAllAgainstFromCounters() {
  const decks = await listDecks();
  let totalAdded = 0, totalRemoved = 0, decksChanged = 0;
  for (const d of decks) {
    const result = await refreshAgainstFromCounters(d.id);
    if (!result) continue;
    if (result.added || result.removed) decksChanged++;
    totalAdded += result.added;
    totalRemoved += result.removed;
  }
  return { decksChecked: decks.length, decksChanged, added: totalAdded, removed: totalRemoved };
}
