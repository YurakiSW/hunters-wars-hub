...
// Un mostro già disambiguato ("Nome (ID xxxxx)") è comunque lo stesso
// mostro del nome pulito — solo scritto diverso perché salvato prima che
// il doppione venisse risolto (vedi "Controlla doppioni collab" in
// Admin). Va tolto SEMPRE prima di decidere se due Difese/Counter sono
// la stessa cosa, altrimenti restano doppioni per sempre anche dopo aver
// sistemato il nome. Aggiunto l'08/08/2026 (Flora — caso Wind Stark).
const stripIdSuffix = (n) => n.replace(/ \(ID \d+\)$/, "");
...
export async function findMatchingDef(monsters, excludeDefId) {
  ...
  const canon = (n) => normalizeMonsterName(canonicalMap.get(normalizeMonsterName(stripIdSuffix(n))) || stripIdSuffix(n));
...
export function counterHasBuildInfo(c) {
  return c.units?.some((u) => u.runes || u.artifactLeft?.length || u.artifactRight?.length);
}

// Corregge il leader di un counter usando il campo `lead` (il nome, sempre
// affidabile dal log — per convenzione in Siege il primo mostro dell'attacco
// è SEMPRE il leader) quando nessuna unità ha ancora `lead:true`. Capita sui
// counter importati automaticamente senza replay completo (vedi
// siegeStats.js: quel percorso lascia `units[].lead` a false per tutti,
// tenendo il leader vero solo in questo campo separato). Segna SOLO
// `lead:true` sull'unità giusta — l'ORDINE dell'array resta esattamente
// quello del log, non si tocca mai qui (un eventuale riordino è
// un'azione separata, a mano, dal form di modifica). Non fa nulla se il
// leader non è riconoscibile tra le unità salvate (nome diverso per
// qualche motivo) — mai una scelta a caso.
export async function fixCounterLeader(counterId) {
  const counter = await getCounter(counterId);
  if (!counter) return { ok: false, error: "Counter non trovato." };
  if (counter.units?.some((u) => u.lead)) return { ok: false, error: "Ha già un leader segnato." };
  if (!counter.lead) return { ok: false, error: "Nessun nome leader salvato su questo counter." };

  const target = normalizeMonsterName(counter.lead);
  const idx = (counter.units || []).findIndex((u) => normalizeMonsterName(u.name) === target);
  if (idx === -1) return { ok: false, error: `"${counter.lead}" non trovato tra le unità del counter.` };

  const units = counter.units.map((u, i) => ({ ...u, lead: i === idx }));
  await updateCounter(counterId, { units });
  return { ok: true };
}
