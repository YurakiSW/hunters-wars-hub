...
// Per estrarre il nome vero da dentro le parentesi del vecchio formato,
// es. "Siege Log Stats (Yuraki)" -> "Yuraki" — usato solo per "approvato
// da", che ora mostra il nome reale di chi ha cliccato approva (non più
// appiattito a "Siege Log" come l'autore).
const LEGACY_WRAPPED_NAME = /^(?:Import Log|Siege Log Stats) \((.+)\)$/;
function extractRealNickname(text) {
  const m = LEGACY_WRAPPED_NAME.exec(text || "");
  return m ? m[1] : text;
}
  ...
  for (const d of defs) {
    ...
    for (const c of d.counters || []) {
      ...
      if (LEGACY_SIEGE_AUTHOR.test(c.authorNickname || "")) {
        patch.authorNickname = "Siege Log";
        touched = true;
      }
      if (LEGACY_SIEGE_AUTHOR.test(c.approvedByNickname || "")) {
        patch.approvedByNickname = extractRealNickname(c.approvedByNickname);
        touched = true;
      }
      if (touched) {
        await redis.set(counterKey(c.id), { ...c, ...patch });
        renamed++;
      }
    }
  }
