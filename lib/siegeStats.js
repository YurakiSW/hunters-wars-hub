...
import { createDef, createCounter, listDefs, deleteCounter, updateCounter } from "./defs";
import { normalizeMonsterName } from "./textUtils";
import { describeRuneSets, describeRuneMainStats } from "./runeSets";
...
export async function approveProposal(defK, counterK, { authorId, authorNickname }, overridePayload) {
  ...
  if (!def) {
    def = await createDef({
      ...
      authorId, authorNickname: "Siege Log",
      autoApprove: true,
    });
  }
  ...
  const counter = await createCounter(
    ...
    { authorId, authorNickname: "Siege Log", autoApprove }
  );
...
export async function resyncApprovedCounters() {
  ...
  for (const d of defs) {
    for (const c of d.counters || []) {
      ...
      if (!best?.units) best = await bestVariant(defK, legacyOrderedTeamKey(c.offense));
      if (!best?.units) continue; // nessun dato grezzo sopravvissuto per questa coppia, in nessuno dei due formati

      const decoded = decodeUnitsForDisplay(best.units);
      let changed = false;
      const newUnits = c.units.map((u, i) => {
        const fresh = decoded[i];
        if (!fresh || fresh.name !== u.name) return u; // ordine diverso da quello atteso, non rischiamo

        const patch = { ...u };
        if (!u.stats && fresh.stats) { patch.stats = fresh.stats; changed = true; }
      ...
      if (changed) {
        await updateCounter(c.id, { units: newUnits });
        updated++;
      }
    }
  }
