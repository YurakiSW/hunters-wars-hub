...
async function refreshProposals(pairs, updatedAggByPair) {
  ...
  // Un counter già pubblicato che mantiene (o migliora) il winrate MA ha
  // anche una build più completa di quella salvata sul sito (rune/artefatti
  // apparsi solo ora, magari perché qualcuno ha aperto il replay dopo
  // l'approvazione) merita comunque "Aggiornamento disponibile" — non solo
  // quando il winrate sale. Si controlla SOLO per le coppie che altrimenti
  // resterebbero "approved" piatte (le altre già lo diventano per conto
  // loro: sotto soglia -> underperforming, winrate in salita -> update
  // già scattato sopra). Guardato solo su queste, non su tutte le coppie,
  // per non appesantire un ricalcolo che gira spesso.
  const buildUpgradeCandidates = pairs
    ...
    .filter(({ p, existing }) => {
      ...
      if (!isPublished || agg.winRate < PROPOSAL_THRESHOLD) return false;
      const alreadyImproving = existing.approvedWinRate != null && agg.winRate > existing.approvedWinRate;
      return !alreadyImproving && existing.counterId;
    });
  const buildUpgradeResults = new Map(); // `${defK}::${counterK}` -> true se ha davvero piu' info ora
  ...
  pairs.forEach((p, i) => {
    ...
    if (isPublished) {
      ...
      } else if (buildUpgradeResults.get(`${p.defK}::${p.counterK}`)) {
...
export async function backfillLogOwnerNicknames() {
  ...
  // Le varianti sono salvate con i nomi CANONICI (le versioni collab e
  // normali confluiscono), mentre i counter possono avere ancora il nome
  // originale: senza canonicalizzare qui, la chiave non combacia e il nick
  // non viene trovato per nessun counter su difese con mostri da collab.
  const canonicalMap = await getCanonicalNameMap();
  const canonNames = (names) => (names || []).map((n) => canonicalMonsterName(n, canonicalMap));
  ...
  for (const d of defs) {
    ...
    for (const c of d.counters || []) {
      ...
      const off = canonNames(c.offense);
      let best = await bestVariant(dK, orderedTeamKey(off));
      if (!best?.ownerNick) best = await bestVariant(dK, legacyOrderedTeamKey(off));
...
export async function resyncApprovedCounters() {
  const defs = await listDefs();
  const baseAccuracyByName = await getBaseAccuracyByName();
  // Come nel backfill dei nick: le varianti usano i nomi canonici, i counter
  // possono avere ancora quelli originali. Senza canonicalizzare, i counter
  // su difese con mostri da collab non troverebbero mai i propri dati.
  const canonicalMap = await getCanonicalNameMap();
  const canonNames = (names) => (names || []).map((n) => canonicalMonsterName(n, canonicalMap));
  let checked = 0;
  let foundData = 0;
  ...
  for (const d of defs) {
    for (const c of d.counters || []) {
      ...
      const defK = defenseKey(canonNames(d.monsters));
      const offNames = canonNames(c.offense);
      let best = await bestVariant(defK, orderedTeamKey(offNames));
      let usedLegacy = false;
      if (!best?.units) {
        best = await bestVariant(defK, legacyOrderedTeamKey(offNames));
        usedLegacy = true;
      }
