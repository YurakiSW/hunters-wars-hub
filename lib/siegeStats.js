  ...
  pairs.forEach((p, i) => {
    ...
    if (!existing) {
      ...
      writePipeline.set(pKey, {
        ...
        // Vittorie/sconfitte e chi ha perso: la percentuale da sola non dice
        // se "67%" sono 2 su 3 o 40 su 60, né chi ci ha provato senza
        // riuscirci (14/08/2026, Flora).
        wins: agg.wins, total: agg.total, losers: agg.losers || null,
    ...
    existing.losers = agg.losers || null;
    const isPublished = existing.status === "approved" || existing.status === "update_available" || existing.status === "underperforming";
    if (isPublished) {
...
export async function recordCrossPlayerBattles(matchups, richUnitsByOffenseDefenseKey) {
  ...
  for (const m of toRecord) {
    ...
    const a = aggDelta.get(pairKey) || { defK, counterK, wins: 0, total: 0, defenseNames: m.defense, offenseNames: m.offense, losers: {}, winners: {} };
    a.total++;
    if (m.win) {
      a.wins++;
      // Chi ha VINTO: serve quando nessuno ha aperto il replay di quella
      // coppia. In quel caso non esiste una build catturata e quindi
      // nemmeno un proprietario, e il counter restava senza nome. Il log
      // leggero però dice comunque chi ha attaccato e se ha vinto
      // (14/08/2026, Flora).
      if (m.wizardName) a.winners[m.wizardName] = (a.winners[m.wizardName] || 0) + 1;
    }
    // Chi ha PERSO su questa coppia (14/08/2026, Flora): le sconfitte non
    // lasciano un replay da cui ricavare la build (il parser prende come
    // attaccante solo il vincitore), quindi senza questo non si saprebbe
    // nemmeno chi ci ha provato senza riuscirci. Serve a capire se un
    // counter "sotto soglia" è davvero debole o se qualcuno l'ha giocato
    // con la build sbagliata.
    else if (m.wizardName) a.losers[m.wizardName] = (a.losers[m.wizardName] || 0) + 1;
  ...
  aggEntries.forEach((a, i) => {
    ...
    // Chi ha perso, sommato a quello che già sapevamo (14/08/2026, Flora).
    const losers = { ...(current.losers || {}) };
    for (const [nome, quante] of Object.entries(a.losers || {})) {
      losers[nome] = (losers[nome] || 0) + quante;
    }
    if (Object.keys(losers).length) updated.losers = losers;
    const winners = { ...(current.winners || {}) };
    for (const [nome, quante] of Object.entries(a.winners || {})) {
      winners[nome] = (winners[nome] || 0) + quante;
    }
    if (Object.keys(winners).length) updated.winners = winners;
...
export async function listProposals(status) {
  ...
  return Promise.all(
    filtered.map(async (p) => {
      ...
      // L'aggregato si legge DAL VIVO invece di fidarsi di quello che era
      // stato copiato dentro la proposal: così vittorie, sconfitte e chi ha
      // perso compaiono subito anche per tutto lo storico, senza dover
      // reimportare i log (14/08/2026, Flora). Reimportare non servirebbe
      // comunque: le battaglie già viste vengono saltate per non contarle
      // due volte.
      const [variants, agg] = await Promise.all([
        allVariants(p.defK, p.counterK),
        redis.get(aggKey(p.defK, p.counterK)),
      ]);
      ...
      return {
        ...
        bestVariant: best,
        variantSummary,
        wins: agg?.wins ?? p.wins ?? null,
        total: agg?.total ?? p.total ?? null,
        losers: agg?.losers ?? p.losers ?? null,
      };
    })
  );
}
...
export async function approveProposal(defK, counterK, { authorId: approverId, authorNickname: approverNickname }, overridePayload) {
  ...
  // Proprietario: prima chi ha giocato la build catturata dal replay. Se
  // per quella coppia nessuno ha aperto il replay non esiste una build, e
  // quindi nemmeno un nome — in quel caso si ripiega su chi ha vinto di più
  // secondo i log leggeri, che il nome ce l'hanno comunque. Meglio "chi
  // l'ha vinta" che nessun nome (14/08/2026, Flora).
  const vincitorePiuFrequente = (vincitori) => {
    const voci = Object.entries(vincitori || {});
    if (!voci.length) return null;
    return voci.sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))[0][0];
  };
  counterData.logOwnerNickname = best?.ownerNick || vincitorePiuFrequente(agg?.winners) || null;
...
export async function recomputeApprovedCounterStats() {
  ...
  for (let i = 0; i < keys.length; i += GRUPPO) {
    ...
    const esiti = await Promise.all(fetta.map(async (key) => {
      ...
      const [counter, best, agg] = await Promise.all([
        getCounter(proposal.counterId),
        bestVariant(proposal.defK, proposal.counterK),
        redis.get(aggKey(proposal.defK, proposal.counterK)),
      ]);
      ...
      // Il flag `lead` DEVE sempre esserci: le varianti importate prima del
      // 14/08/2026 non ce l'hanno (aggiunto all'import solo allora), quindi
      // rigenerando le unità da lì il leader spariva e i counter finivano
      // tutti in "Counter approvati senza leader segnato".
      // Tre reti in cascata, l'ultima non può fallire: il nome salvato sul
      // counter, poi quello della variante, infine il primo mostro — che in
      // Siege è sempre il leader (verificato su 462 squadre del log 11/08).
      const leadName = counter.lead
        || best.units.find((u) => u.lead)?.name
        || decoded[0]?.name
        || null;
      const units = leadName
        ? decoded.map((u) => ({ ...u, lead: normalizeMonsterName(u.name) === normalizeMonsterName(leadName) }))
        : decoded;
      // Riempie anche il proprietario se manca: i counter di coppie dove
      // nessuno ha aperto il replay restavano senza nome, ma i log leggeri
      // dicono comunque chi ha vinto (14/08/2026, Flora).
      const vincitori = Object.entries(agg?.winners || {});
      const vincitorePiuFrequente = vincitori.length
        ? vincitori.sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))[0][0]
        : null;
      await updateCounter(counter.id, {
        units,
        lead: leadName || counter.lead || null,
        logOwnerNickname: counter.logOwnerNickname || best?.ownerNick || vincitorePiuFrequente || null,
