...
import { describeRuneSets } from "./runeSets";
...
function decodeUnitsForDisplay(units) {
  ...
  return units.map((u) => {
    ...
    return {
      ...
      stats: u.stats || "", statsFlexible: u.statsFlexible || false, statsMinText: u.statsMinText || "",
...
const AGG_INDEX_KEY = "siege_agg:index"; // set di OGNI coppia defK::counterK mai toccata (anche sotto soglia) — serve per pulire tutto a Fine Season
...
function buildHashFromUnits(units) {
  ...
  const parts = (units || []).map((u) => {
    if (u.rawRunes || u.rawArtifacts) {
      return JSON.stringify([u.rawRunes || [], u.rawArtifacts || []]);
    }
...
export async function recordCrossPlayerBattles(matchups, richUnitsByOffenseDefenseKey) {
  ...
  const touchedPairKeys = new Set(); // "defK::counterK" — OGNI coppia toccata in questo giro, sopra o sotto soglia
  aggEntries.forEach((a, i) => {
    ...
    touchedPairKeys.add(`${a.defK}::${a.counterK}`);
  });
  ...
  if (touchedPairKeys.size) writePipeline.sadd(AGG_INDEX_KEY, ...touchedPairKeys);
...
export async function archiveAndClearSeason(seasonId) {
  ...
  // Pulizia tabelle live. IMPORTANTE: usiamo AGG_INDEX_KEY (ogni coppia MAI
  // toccata, sopra o sotto soglia) e non solo le proposal — una coppia che
  // non ha mai raggiunto la soglia, o la cui proposal è stata cancellata
  // del tutto, altrimenti resterebbe orfana per sempre e si risommerebbe
  // ai dati del prossimo import (bug scoperto il 29/07/2026).
  const allPairKeys = await redis.smembers(AGG_INDEX_KEY);
  const deletePipeline = redis.pipeline();
  for (const pairKey of allPairKeys) {
    const [defK, counterK] = pairKey.split("::");
    deletePipeline.del(aggKey(defK, counterK));
    const variantHashes = await redis.smembers(variantIndexKey(defK, counterK));
    for (const h of variantHashes) deletePipeline.del(variantKey(defK, counterK, h));
    deletePipeline.del(variantIndexKey(defK, counterK));
    deletePipeline.del(proposalKey(defK, counterK));
  }
  if (allPairKeys.length) await deletePipeline.exec();
  await redis.del(AGG_INDEX_KEY);
