...
export async function recomputeApprovedCounterStats() {
  ...
  for (const key of keys) {
    ...
    if (!proposal?.counterId) continue;

    const counter = await getCounter(proposal.counterId);
    if (!counter) continue;
    if (counter.manuallyEdited) { saltatiManuali++; continue; }

    // Le unità decodificate non conservano le rune grezze: l'unica fonte da
    // cui rigenerare è la variante migliore della coppia.
    const best = await bestVariant(proposal.defK, proposal.counterK);
    if (!best?.units) { senzaDati++; continue; }

    const units = decodeUnitsForDisplay(best.units, baseStatsByName);
