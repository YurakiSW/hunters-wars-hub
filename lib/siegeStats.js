...
function decodeUnitsForDisplay(units) {
  ...
  return units.map((u) => {
    ...
    return {
      ...
      notes: u.notes || [""],
      spd: u.rawSpd ?? null,
      combatStats: u.combatStats || computeCombatStats(u.rawRunes, u.rawCombatBase),
    };
  });
}
    ...
    for (const c of d.counters || []) {
      ...
      if (best?.units) {
        ...
        newUnits = newUnits.map((u, i) => {
          ...
          if (!u.combatStats && fresh.combatStats) { patch.combatStats = fresh.combatStats; changed = true; }
