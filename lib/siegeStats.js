...
export async function listProposals(status) {
  ...
  return Promise.all(
    filtered.map(async (p) => {
      const [best, variants] = await Promise.all([
        bestVariant(p.defK, p.counterK),
      ...
      const variantSummary = variants.map((v) => ({
        ...
        isBest: best ? v.buildHash === best.buildHash : false,
      }));
...
export async function approveProposal(defK, counterK, { authorId: approverId, authorNickname: approverNickname }, overridePayload) {
  ...
  // Segna anche `lead:true` sull'unità giusta dentro `units[]` (non solo il
  // nome separato in `counter.lead`) quando l'approvazione arriva dritta dal
  // log, senza passare da "Modifica e approva" — il leader lo sappiamo già
  // con certezza dalla posizione 0 del log (convenzione fissa di Siege), non
  // c'è nulla da indovinare. Corretto l'08/08/2026 (Flora): prima
  // `units[].lead` restava sempre false su questo percorso, anche quando il
  // nome giusto era già salvato altrove. NON tocca mai l'ordine dell'array,
  // solo il flag — l'ordine resta quello del log.
  const finalUnits = overridePayload
    ? units
    : units.map((u) => ({ ...u, lead: normalizeMonsterName(u.name) === normalizeMonsterName(lead) }));
  ...
  const counterData = {
    ...
    lead,
    turnOrder: overridePayload?.turnOrder || turnOrderBySpd(finalUnits, proposal.offenseNames),
    units: finalUnits,
