...
export function computeCombatStats(rawRunes, combatBase, rawArtifacts, rawRelics, baseStats, teamBonus) {
  ...
  // Grogen di Flora, Fatal x4: sommando usciva 2813, separando 2814 come in
  // gioco). Vale per il blocco SCHEDA.
  const withPct = (base, flatVal, ...parts) =>
    base == null ? null : base + parts.reduce((s, p) => s + Math.ceil((base * (p || 0)) / 100), 0) + flatVal;

  // Per le stat di COMBATTIMENTO vale invece la regola opposta: torre,
  // leader skill e set si sommano TUTTE e si arrotonda una volta sola.
  //   ceil(base × (1 + %set + %leader + %torre)) + rune
  // Verificato sul team Mihyang/Adriana/Rigna di Flora (lead Mihyang +28%
  // SPD): con un solo ceil escono 343/380/352, identici al gioco; con i ceil
  // separati uscivano 345/381/352 (14/08/2026).
  const withPctSum = (base, flatVal, ...parts) =>
    base == null ? null : base + Math.ceil((base * parts.reduce((s, p) => s + (p || 0), 0)) / 100) + flatVal;
  ...
  return {
    ...
    // Quanta SPD arriva SOLO dalle rune (stat principale + sotto-stat +
    // macine). Artefatti e relic non danno mai SPD piatta, quindi questo
    // numero è puramente "quanto mi danno le rune" — il pezzo su cui si
    // lavora davvero quando si fa speed tuning (14/08/2026, Flora).
    spdFromRunes: flat.spd || null,
    critRate, critDmg,
    ...
    // set gilda, tutti sommati in un unico arrotondamento. È da qui che esce
    // l'ordine turni vero.
    hpCombat: withPctSum(hpBase, flat.hp, pct.hp, setPct.hp, tb.hp),
    atkCombat: withPctSum(combatBase.atk, flat.atk, pct.atk, setPct.atk, tb.atk),
    defCombat: withPctSum(combatBase.def, flat.def, pct.def, setPct.def, tb.def),
    spdCombat: withPctSum(combatBase.spd, flat.spd, pct.spd, setPct.spd, tb.spd, TOWER_SPD_BONUS_PCT),
