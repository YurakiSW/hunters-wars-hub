...
// Prende le 6 rune grezze (con pri_eff/sec_eff/prefix_eff) e le stat finali
// già calcolate dal replay, e restituisce il blocco "COMBAT STATS" completo
// per un mostro, con CRIT Rate/Dmg/Accuracy ricalcolati a mano (base +
// rune + bonus set), tutto il resto preso diretto dal replay.
export function computeCombatStats(rawRunes, combatBase) {
  ...
  for (const [setId, bonus] of Object.entries(SET_BONUS_CRIT_ACC)) {
    if ((setCounts[setId] || 0) >= bonus.threshold) {
      if (bonus.key === "critRate") critRate += bonus.value;
      else if (bonus.key === "critDmg") critDmg += bonus.value;
      else if (bonus.key === "accuracy") accuracy += bonus.value;
    }
  }

  return {
    hp: combatBase.hp ?? null,
    atk: combatBase.atk ?? null,
    def: combatBase.def ?? null,
    spd: combatBase.spd ?? null,
    critRate, critDmg,
    resistance: combatBase.resistance ?? null,
    accuracy,
  };
}
